    ...
    public function ipn(Request $request)
    {
        ...
        if ($response) {
            ...
            if ($result) {
                if ($result['data']) {
                    ...
                    if ($result['data']['status'] == 'success') {
                        ...
                        if ($am == $sam && $result['data']['currency'] == $deposit->method_currency  && $deposit->status == '0') {
                            ...
                            $notify[] = ['success', __('Payment captured successfully')];
                            return to_route(gatewayRedirectUrl(true))->withNotify($notify);
                        } else {
                            $notify[] = ['error', __('Less amount paid. Please contact with admin.')];
                        }
                    } else {
                ...
                $notify[] = ['error', __('Something went wrong while executing')];
            }
        } else {
            $notify[] = ['error', __('Something went wrong while executing')];
        }
