...
class UserController extends Controller
{
    ...
    public function create2fa(Request $request)
    {
        ...
        if ($response) {
            ...
            $user->ts  = 1;
            $user->save();
            $notify[] = ['success', __('Google authenticator activated successfully')];
    ...
    public function disable2fa(Request $request)
    {
        ...
        if ($response) {
            ...
            $user->ts  = 0;
            $user->save();
            $notify[] = ['success', __('Two factor authenticator deactivated successfully')];
        } else {
            $notify[] = ['error', __('Wrong verification code')];
        }
        return back()->withNotify($notify);
    ...
    {
        if (auth()->user()->kv == 2) {
            $notify[] = ['error', __('Your KYC is under review')];
            return to_route('user.home')->withNotify($notify);
        }
        if (auth()->user()->kv == 1) {
            $notify[] = ['error', __('You are already KYC verified')];
            return to_route('user.home')->withNotify($notify);
        }
        $pageTitle = __('KYC Form');
    ...
    public function kycSubmit(Request $request)
    {
        ...
        $notify[] = ['success', __('KYC data submitted successfully')];
        return to_route('user.home')->withNotify($notify);
    ...
    public function userData()
    {
        ...
        $notify[] = ['success', __('Registation Successful..')];
        return to_route('user.home')->withNotify($notify);
    }
    ...
    public function userDataSubmit(Request $request)
    {
        ...
        $notify[] = ['success', __('Registration process completed successfully')];
        return to_route('user.home')->withNotify($notify);
    }
    ...
    public function promotionalBanners()
    {
        ...
        $pageTitle    = __('Promotional Banners');
        $banners      = PromotionTool::orderBy('id', 'desc')->get();
        $emptyMessage = __('No banner found');
    ...
    public function transferBalanceSubmit(Request $request)
    {
        ...
        if ($user->username == $request->username) {
            $notify[] = ['error', __('You cannot transfer balance to your own account')];
            return back()->withNotify($notify);
        }

        $receiver = User::where('username', $request->username)->first();
        if (!$receiver) {
            $notify[] = ['error', __('Oops! Receiver not found')];
            return back()->withNotify($notify);
        }
        ...
        if ($user->$wallet < $afterCharge) {
            $notify[] = ['error', __('You have no sufficient balance to this wallet')];
            return back()->withNotify($notify);
        }
        ...
        $transaction->details      = __('Balance transfer to ') . $receiver->username;
        ...
        $transaction->details      = __('Balance received from ') . $user->username;
        ...
        $notify[] = ['success', __('Balance transferred successfully')];
        return back()->withNotify($notify);
    }
    ...
    public function findUser(Request $request)
    {
        ...
        if (@$user->username == auth()->user()->username) {
            $message = __('You cannot send money to your own account');
        }
