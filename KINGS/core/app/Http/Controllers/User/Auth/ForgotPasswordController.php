...
class ForgotPasswordController extends Controller
{
    ...
    */
    ...
    public function sendResetCodeEmail(Request $request)
    {
        ...
        if (!$user) {
            $notify[] = ['error', __('Couldn\'t find any account with this information')];
        ...
        $notify[] = ['success', __('Password reset email sent successfully')];
    ...
    public function codeVerify(){
        ...
        if (!$email) {
            $notify[] = ['error',__('Oops! session expired')];
    ...
    public function verifyCode(Request $request)
    {
        ...
        if (PasswordReset::where('token', $code)->where('email', $request->email)->count() != 1) {
            $notify[] = ['error', __('Verification code doesn\'t match')];
            return to_route('user.password.request')->withNotify($notify);
        }
        $notify[] = ['success', __('You can change your password.')];
