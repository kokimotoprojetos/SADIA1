    ...
    public function invest(Request $request)
    {
        ...
        if($plan->fixed_amount > 0){
            if ($amount != $plan->fixed_amount) {
                $notify[] = ['error',__('Please check the investment limit')];
        ...
        if ($request->amount > $user->$wallet) {
            $notify[] = ['error', __('Your balance is not sufficient')];
            return back()->withNotify($notify);
        }
        ...
        $notify[] = ['success',__('Invested to plan successfully')];
        return back()->withNotify($notify);
    }

    public function statistics()
    {
        $pageTitle = __('Invest Statistics');
