...
class HyipLab
{
    ...
    */
    public function invest($amount, $wallet){
        ...
        $transaction->details       = __('Invested on') . ' ' . $plan->name;
        $transaction->trx           = $trx;
        ...
        notify($user, 'INVESTMENT', [
            ...
            'time' => $plan->lifetime == 1 ? __('lifetime') : $plan->repeat_time.' '.__('times'),
        ...
        $adminNotification->title = $this->setting->cur_sym.showAmount($amount).' '.__('invested to').' '.$plan->name;
    ...
    public static function levelCommission($user, $amount, $commissionType, $trx, $setting){
        ...
        while ($i <= $level) {
            ...
            $transactions[] = [
                ...
                'charge' => 0,
                'trx_type' => '+',
                'details' => __('level').' '.$i.' '.__('Referral Commission From').' '.$user->username,
                'trx' => $trx,
