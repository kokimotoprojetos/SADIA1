...
class GeneralSettingController extends Controller
{
    ...
    public function offDaySubmit(Request $request){
        ...
        if ($totalOffDay == 7) {
            $notify[] = ['error',__('You couldn\'t set all days as holiday')];
            return back()->withNotify($notify);
        }
        $general = GeneralSetting::first();
        $general->off_day = $request->off_day;
        $general->save();
        $notify[] = ['success',__('Weekly Holiday Setting Updated')];
        return back()->withNotify($notify);
    }
    ...
    public function holidaySubmit(Request $request)
    {
        ...
        $holiday->save();
        $notify[] = ['success',__('Holiday added successfully')];
        return back()->withNotify($notify);
    }
    ...
    public function remove($id)
    {
        ...
        $holiday->delete();
        $notify[] = ['success',__('Holiday deleted successfully')];
        return back()->withNotify($notify);
    }
}
