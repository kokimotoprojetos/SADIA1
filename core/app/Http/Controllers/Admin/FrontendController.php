...
class FrontendController extends Controller
{
    ...
    public function templatesActive(Request $request)
    {
        ...
        $general->save();
        $notify[] = ['success', __(strtoupper($request->name) . ' template activated successfully')];
    ...
    public function templateUpload(Request $request)
    {
        ...
        if(!extension_loaded('zip')){
            $notify[]=['error',__('zip Extension is required to install the template')];
            return back()->withNotify($notify);
        }
        ...
        if (!file_exists($dir . '/config.json')) {
            $this->removeDir($dir);
            $notify[] = ['error', __('Config file not found')];
            return back()->withNotify($notify);
        }
        ...
        if ($extract == false) {
            $this->removeDir($dir);
            $notify[] = ['error', __('Something went wrong to extract')];
            return back()->withNotify($notify);
        }
        ...
        foreach ($temPaths as $temp) {
            ...
            if ($tempname == $config->name) {
                $this->removeDir($dir);
                $notify[] = ['error', __('Template already exists')];
                return back()->withNotify($notify);
            }
        }
        ...
        if (!\Hash::check(systemDetails()['h_verifier'], $config->hash)) {
            $this->removeDir($dir);
            $notify[] = ['error', __('Template hash is invalid')];
            return back()->withNotify($notify);
        }
        ...
        if (!file_exists($mainFile)) {
            $this->removeDir($dir);
            $notify[] = ['error', __('Main file not found')];
            return back()->withNotify($notify);
        }
        ...
        $notify[] = ['success', __('Template uploaded successfully')];
        return back()->withNotify($notify);
    }
    ...
    public function remove($id)
    {
        ...
        $frontend->delete();
        $notify[] = ['success', __('Content removed successfully')];
        return back()->withNotify($notify);
    }
    ...
    public function importContent(Request $request, $key)
    {
        ...
        if (!array_key_exists($key, $fromTempJson)) {
            $notify[] = ['error', __('Key doesn\'t exists')];
            return back()->withNotify($notify);
        }
