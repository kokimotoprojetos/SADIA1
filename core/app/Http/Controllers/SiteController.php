...
class SiteController extends Controller
{
    ...
    public function subscribe(Request $request)
    {
        ...
        $notify = __('Thank you, we will notice you our latest news');
    ...
    public function planCalculator(Request $request)
    {
        if ($request->planId == null) {
            return response(['errors' => __('Please Select a Plan!')]);
        }
        $requestAmount = $request->investAmount;
        if ($requestAmount == null || 0 > $requestAmount) {
            return response(['errors' => __('Please Enter Invest Amount!')]);
        }
        $general = gs();
        $plan    = Plan::where('id', $request->planId)->where('status', 1)->first();
        if (!$plan) {
            return response(['errors' => __('Invalid Plan!')]);
        }

        if ($plan->fixed_amount == '0') {
            if ($requestAmount < $plan->minimum) {
                return response(['errors' => __('Minimum Invest') . ' ' . getAmount($plan->minimum) . ' ' . $general->cur_text]);
            }
            if ($requestAmount > $plan->maximum) {
                return response(['errors' => __('Maximum Invest') . ' ' . getAmount($plan->maximum) . ' ' . $general->cur_text]);
            }
        } else {
            if ($requestAmount != $plan->fixed_amount) {
                return response(['errors' => __('Fixed Invest amount') . ' ' . getAmount($plan->fixed_amount) . ' ' . $general->cur_text]);
            }
        }
        ...
        if ($plan->lifetime == 0) {
            ...
            $total      = ($interestAmount * $plan->repeat_time) . ' ' . $general->cur_text;
            ...
            $result['description'] = __('Return') . ' ' . showAmount($interestAmount) . ' ' . $general->cur_text . ' ' . __('Every') . ' ' . $timeName->name . ' ' . __('For') . ' ' . $ret . ' ' . $timeName->name . '. ' . __('Total') . ' ' . $total;
            $result['totalMoney']  = $totalMoney;
            $result['netProfit']   = $totalMoney - $request->investAmount;
        } else {
            $result['description'] = __('Return') . ' ' . showAmount($interestAmount) . ' ' . $general->cur_text . ' ' . __('Every') . ' ' . $timeName->name . ' ' . __('For Lifetime');
        }
    ...
    public function getDeviceToken(Request $request){
        ...
        return ['success'=>true, 'message'=>__('Token save successfully')];
    }
