...
class WithdrawController extends Controller
{
    ...
    public function withdrawStore(Request $request)
    {
        ...
        $isHoliday = HyipLab::isHoliDay(now()->toDateTimeString(), gs());
        if ($isHoliday && !gs()->holiday_withdraw) {
            $notify[] = ['error', __('Today is holiday. You\'re unable to withdraw today')];
        ...
        if ($request->amount < $method->min_limit) {
            $notify[] = ['error', __('Your requested amount is smaller than minimum amount.')];
            return back()->withNotify($notify);
        }
        if ($request->amount > $method->max_limit) {
            $notify[] = ['error', __('Your requested amount is larger than maximum amount.')];
            return back()->withNotify($notify);
        }

        if ($request->amount > $user->interest_wallet) {
            $notify[] = ['error', __('You do not have sufficient balance for withdraw.')];
            return back()->withNotify($notify);
        }
    ...
    public function withdrawSubmit(Request $request)
    {
        ...
        if ($withdraw->amount > $user->interest_wallet) {
            $notify[] = ['error', __('Your request amount is larger then your current balance.')];
            return back()->withNotify($notify);
        }
        ...
        $transaction->details      = showAmount($withdraw->final_amount) . ' ' . $withdraw->currency . __(' Withdraw Via ') . $withdraw->method->name;
        ...
        $notify[] = ['success', __('Withdraw request sent successfully')];
        return to_route('user.withdraw.history')->withNotify($notify);
    }

    public function withdrawLog(Request $request)
    {
        $pageTitle = __("Withdraw Log");
