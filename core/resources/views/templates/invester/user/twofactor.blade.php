...
<div class="my-head">
    <div class="my-head-con">
        <div class="my-head-item" id="avatar">
            <img src="{{ getImage(getFilePath('logoIcon').'/logo_2.png') }}"/>
        </div>
        <div class="my-head-item">
            <h3>UID:10425{{ auth()->user()->id }}</h3>
            <span>{{ auth()->user()->username }}</span>
            <input style="width:70%; background:none; outline:none; border:none;" type="text" value="{{ route('home') }}?reference={{ auth()->user()->username }}" class="copyURL copy">
       
                <small style="margin-left:5px; background-color:red; border-radius:4px; height:18px;" class="copyBoard copyText" id="copyBoard" >{{ @lang('COPY') }}</small>
            </span>
        </div>
    </div>
    @php
...
@php
$authUser = Auth::user();
 $totalDeposit=  App\Models\Deposit::where('user_id', $authUser->id)->where('status', 1)->sum('amount');
 @endphp
@php
$authUser = Auth::user();
 $totalWithdraw=  App\Models\Withdrawal::where('user_id', $authUser->id)->whereIn('status', [1])->sum('amount');
 @endphp
@php
...
@endphp
    ...
    <ul class="my-asset">
        ...
        <li>
            <label>{{ @lang('Withdraw') }}</label>
            <span>{{ $general->cur_sym }} {{ getAmount($totalWithdraw) }}</span>
        </li>
        <li>
            <label>{{ @lang('Deposit') }}</label>
            <span>{{ $general->cur_sym }} {{ getAmount($totalDeposit) }}</span>
        </li>
        <li>
            <label>{{ @lang('Today Income') }}</label>
            <span>0</span>
        </li>
        <li>
            <label>{{ @lang('My Teams') }}</label>
            <span>{{$userCount}}</span>
        </li>
        <li>
            <label>{{ @lang('Team Income') }}</label>
            <span>{{ $general->cur_sym }} {{getAmount($referralCommission)}}</span>
        </li>
    </ul>

    <ul class="my-balance">
        <li>
            <span>{{ showAmount(auth()->user()->interest_wallet) }}</span>
            <label>{{ @lang('Balance Wallet') }}</label>
        </li>
        <li>
            <span>{{ showAmount(auth()->user()->deposit_wallet) }}</span>
            <label>{{ @lang('Recharge Wallet') }}</label>
        </li>
    </ul>
    <div class="my-button">
        <a href="{{ route('user.withdraw') }}">{{ @lang('Withdraw') }}</a>
        <a href="{{ route('user.deposit.index') }}">{{ @lang('Recharge') }}</a>
    </div>
    <div class="my-nav">
        <ul>
            <li>
            ...
            <li>
                <a href="">
                    <img src="/static/home/img/user07.png"/>
                    <span>{{ @lang('Download APP') }}</span>
                </a>
            </li>
        </ul>
        <div class="space-20"></div>
        <a href="{{ route('user.logout') }}" class="logout">{{ @lang('Sign Out') }}</a>
    </div>
    <div class="fixd-icons">
        <a href="" target="_blank" class="kf-item">
            <img src="/static/home/img/tg.png">
        </a>
                            <a href="https://chat.whatsapp.com/BBCobSUn1C9F8r0wGtUCaf" target="_blank" class="kf-item">
                <img src="/static/home/img/ws.png">
            </a>
            </div>
...
.treasure-record .treasure-dd span:last-child {
    ...
    var _token = "eP44G9VqYlmhISm3Ed4hMjeTgEVUuK8UJgESIg2g";
    layui.use(['upload', 'form', 'layer'], function () {
        $ = layui.jquery;
        var form = layui.form, layer = layui.layer, upload = layui.upload;
        upload.render({
            elem: '#avatar',
            url: "https://coworker-ng.com/common/image",
            accept: 'file',
            auto: true,
            multiple: false,
                ...
                switch (result.code) {
                    ...
                    case 201:
                        layer.msg('login invalid', {icon: 16, time: 1000, shade: [0.5, '#393D49']}, function () {
                            location.href = "https://coworker-ng.com/login?redirect=https%3A%2F%2Fcoworker-ng.com%2Fuser";
                        });
                        break;
                    default:
        ...
        $("body").on('click', '.my-obtain button', function () {
            var _this = $(this);
            _this.attr('disabled', true);
            var loading = layer.load(2, {shade: [0.5, '#393D49']});
            $.post("https://coworker-ng.com/order/done", {_token: _token}, function (result) {
                layer.close(loading);
    ...
    function setAvatar(avatar) {
        ...
        $.post("https://coworker-ng.com/user/avatar", data, function (result) {
            layer.close(loading);
            switch (result.code) {
                case 200:
                    layer.msg('success', {icon: 16, time: 1000, shade: [0.5, '#393D49']}, function () {
    ...
    copy.on('success', function (e) {
        e.clearSelection();
        layer.msg('copy success', {icon: 16, time: 500, shade: [0.5, '#393D49']});
    });

    copy.on('error', function (e) {
        layer.msg('copy fail', {icon: 16, time: 1000, shade: [0.5, '#393D49']});
    });
</script>
</body>
